...
- (void)buildTransitionComposition:(AVMutableComposition *)composition andVideoComposition:(AVMutableVideoComposition *)videoComposition andAudioMix:(AVMutableAudioMix *)audioMix{
    CMTime nextCilpStartTime = kCMTimeZero;
    NSInteger i;
    ...
    compositionAudioTracks[0] = [composition addMutableTrackWithMediaType:AVMediaTypeAudio preferredTrackID:kCMPersistentTrackID_Invalid]; // 添加视频轨道0
    compositionAudioTracks[1] = [composition addMutableTrackWithMediaType:AVMediaTypeAudio preferredTrackID:kCMPersistentTrackID_Invalid]; // 添加视频轨道1
    ...
    //在合成中将剪辑放入交替的视频和音频轨道中，并由transitionDuration重叠。
    for (i = 0; i<clipsCount; i++) {
        NSInteger alternatingIndex = i % 2;//交替标志:0,1,0,1...
        AVURLAsset *asset = self.clips[i];
        NSValue *clipTimeRange = self.clipTimeRanges[i];
        CMTimeRange timeRangInAsset;
        if (clipTimeRange) {
            timeRangInAsset = [clipTimeRange CMTimeRangeValue];
        }else{
            timeRangInAsset = CMTimeRangeMake(kCMTimeZero, [asset duration]);
        }

        AVAssetTrack *clipVideoTrack = [[asset tracksWithMediaType:AVMediaTypeVideo] objectAtIndex:0];
        NSError *error;
        [compositionVideoTracks[alternatingIndex] insertTimeRange:timeRangInAsset ofTrack:clipVideoTrack atTime:nextCilpStartTime error:&error];

        AVAssetTrack *clipAudioTrack = [[asset tracksWithMediaType:AVMediaTypeAudio]objectAtIndex:0];
        [compositionAudioTracks[alternatingIndex] insertTimeRange:timeRangInAsset ofTrack:clipAudioTrack atTime:nextCilpStartTime error:&error];
        NSLog(@"add at %lf long %lf",CMTimeGetSeconds(timeRangInAsset.start) + CMTimeGetSeconds(nextCilpStartTime),CMTimeGetSeconds(timeRangInAsset.duration));

        //计算应该直接播放的区间
        //从播放区间里面去掉变换区间
        passThroughTimeRanges[i] = CMTimeRangeMake(nextCilpStartTime, timeRangInAsset.duration);
        if (i > 0) {
        ...
        //计算下一个插入点
        nextCilpStartTime = CMTimeAdd(nextCilpStartTime, timeRangInAsset.duration);//加上持续时间
        nextCilpStartTime = CMTimeSubtract(nextCilpStartTime, transitionDuration);//减去变换时间,得到写一个插入点
        //从第i个视频的变换时间为下一个的插入点,长度为变换时间
        if (i+1 < clipsCount) {
            transitionTimeRanges[i] = CMTimeRangeMake(nextCilpStartTime, transitionDuration);
        }
        NSLog(@"transitionTimeRange at %lf long %lf",CMTimeGetSeconds(transitionTimeRanges[i].start),CMTimeGetSeconds(transitionTimeRanges[i].duration));
    }
    ...
    for (i = 0; i<clipsCount; i++) {
        ...
        [instructions addObject:passThroughInstruction];//添加到指令合集
