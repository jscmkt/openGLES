//
//  GPUandInstructionViewController.m
//  learnGPUImage
//
//  Created by you&me on 2019/9/23.
//  Copyright © 2019 you&me. All rights reserved.
//

#import "GPUandInstructionViewController.h"
#import "SimpleEditor.h"
#import <GPUImage.h>
#import <AssetsLibrary/AssetsLibrary.h>
#import <GPUImageMovieComposition.h>
@interface GPUandInstructionViewController ()
...
@property(nonatomic,strong) GPUImageMovieComposition *imageMovieCompostion;

@end

@implementation GPUandInstructionViewController{
...
-(void)setupEditingAndPlayback{
    ...
    AVURLAsset *asset2 = [AVURLAsset assetWithURL:[NSURL fileURLWithPath:[[NSBundle mainBundle]pathForResource:@"def" ofType:@"mp4"]]];
...
-(void)synchronizePlayerWithEditor{
    ...
    self.imageMovieCompostion = [[GPUImageMovieComposition alloc]initWithComposition:self.editor.composition andVideoComposition:self.editor.videoComposition andAudioMix:self.editor.audioMix];
    self.imageMovieCompostion.runBenchmark = YES;

    [self.imageMovieCompostion addTarget:self.movieWriter];
    [self.imageMovieCompostion addTarget:filter];

    [self.imageMovieCompostion enableSynchronizedEncodingUsingMovieWriter:self.movieWriter];
    self.imageMovieCompostion.audioEncodingTarget = self.movieWriter;

    [_movieWriter startRecording];
    [self.imageMovieCompostion startProcessing];
    ...
    [_movieWriter setCompletionBlock:^{
        ...
        [strongSelf->_imageMovieCompostion endProcessing];
...
-(void)synchronizeWithEditor{
    //clips
    [self synchronizeEditorClipsWothOurClips];
    [self synchronizeEditorClipsWothOurClips];
    self.editor.transitionDuration = CMTimeMakeWithSeconds(1, 600);
    [self.editor buildCompositionObjectForPlayback];
    [self synchronizePlayerWithEditor];
}

-(void)synchronizeEditorClipsWothOurClips{
