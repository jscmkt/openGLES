...
//    NSCache *framebufferCache;
    NSMutableDictionary *framebufferCache;///< 缓存字典
    NSMutableDictionary *framebufferTypeCounts;///< 缓存数量字典
    NSMutableArray *activeImageCaptureList; ///< 正在读取Image数据的GPUImageFrameBuffer列表 // Where framebuffers that may be lost by a filter, but which are still needed for a UIImage, etc., are stored
    id memoryWarningObserver;

    dispatch_queue_t framebufferCacheQueue;//缓存队列
}

- (NSString *)hashForSize:(CGSize)size textureOptions:(GPUTextureOptions)textureOptions onlyTexture:(BOOL)onlyTexture;///< 根据size,textureOption和onlyTexture,创建缓存字符串.缓存字符串+当前缓存数量行成framebufferCache缓存的key.如果找不到framebufferCache对应的数量,会创建新的缓存
...
- (void)returnFramebufferToCache:(GPUImageFramebuffer *)framebuffer;///回收缓存.根据size,textureOptions和onlyTexture,创建缓存字符串,缓存字符串+缓存数量形成framebufferCachec缓存的key(之所以加上数量,是因为缓存字符串不唯一)
{
