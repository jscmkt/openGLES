...
- (id)initWithCGImage:(CGImageRef)newImageSource smoothlyScaleOutput:(BOOL)smoothlyScaleOutput;
{
    ...
    if (!shouldRedrawUsingCoreGraphics) {
            ...
            CGImageGetBitsPerComponent(newImageSource) != 8)
        {
            ...
            /* Check that the bitmap pixel format is compatible with GL *///检查位图像素格式是否与GL兼容
            CGBitmapInfo bitmapInfo = CGImageGetBitmapInfo(newImageSource);
            if ((bitmapInfo & kCGBitmapFloatComponents) != 0) {
                /* We don't support float components for use directly in GL *///我们不支持直接在GL中使用浮动组件
                shouldRedrawUsingCoreGraphics = YES;
            } else {
                CGBitmapInfo byteOrderInfo = bitmapInfo & kCGBitmapByteOrderMask;
                if (byteOrderInfo == kCGBitmapByteOrder32Little) {
                    /* Little endian, for alpha-first we can use this bitmap directly in GL *///对于alpha-first，我们可以在GL中直接使用这个位图
